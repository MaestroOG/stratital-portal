...
const ProjectCard = ({ title, desc, success, yellow, number, onClick, className }) => {
    return (
        <div onClick={onClick} className={`w-full md:max-w-[367px] bg-white cursor-pointer px-5 py-4 rounded-2xl flex items-center justify-between max-sm:border max-sm:border-gray-300 max-sm:rounded-2xl max-sm:shadow-lg ${className ?? className}`}>
